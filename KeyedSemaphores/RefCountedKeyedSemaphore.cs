...
namespace KeyedSemaphores
{
    ...
    internal readonly struct RefCountedKeyedSemaphore<TKey>
        : IEquatable<RefCountedKeyedSemaphore<TKey>>,
            IDisposable
        where TKey : notnull
    {
        ...
        public RefCountedKeyedSemaphore(
            TKey key,
            ConcurrentDictionary<TKey, RefCountedKeyedSemaphore<TKey>> dictionary
        )
        {
        ...
        public static bool operator ==(
            RefCountedKeyedSemaphore<TKey> left,
            RefCountedKeyedSemaphore<TKey> right
        )
        {
        ...
        internal sealed class SemaphoreReleaser : IDisposable
        {
            ...
            public SemaphoreReleaser(
                ...
                ConcurrentDictionary<TKey, RefCountedKeyedSemaphore<TKey>> keyedSemaphores
            )
            {
                _key = key;
                ...
                while (true)
                {
                    if (!_keyedSemaphores.TryGetValue(_key, out var existingKeyedSemaphore))
                    {
                        throw new InvalidOperationException(
                            $"Did not expect the keyed semaphore with key {_key} to already be removed from the dictionary"
                        );
                    }
                    ...
                    if (existingKeyedSemaphore._refs == 1)
                    {
                        ...
                        var kvp = new KeyValuePair<TKey, RefCountedKeyedSemaphore<TKey>>(
                            _key,
                            existingKeyedSemaphore
                        );
                        ...
                        if (
                            (
                                (ICollection<KeyValuePair<TKey, RefCountedKeyedSemaphore<TKey>>>)
                                    _keyedSemaphores
                            ).Remove(kvp)
                        )
                        {
